#Arrays----------------------------------------------------------------------------------------------------
  ...
  vul.total.obs        <- matrix(0, fishery.yrs, 1)
  index.expect         <- matrix(0, fishery.yrs, 1)
  survey.catch.age.len <- array(NA,c(fishery.yrs, ages, length(len.step), sexes))
  
  #Storage matrices for data samples
  f.lengths            <- matrix(0, fishery.yrs, (2*length(len.step)))
  s.lengths            <- matrix(0, fishery.yrs, (2*length(len.step)))
  #f.a.ca               <- matrix(0, fishery.yrs, (ages*2-2))
  #s.a.ca               <- matrix(0, fishery.yrs, (ages*2-2))
  f.a.ca               <- matrix(0, fishery.yrs, (max.age*2))
  s.a.ca               <- matrix(0, fishery.yrs, (max.age*2))
  f.sample.size        <- rep(0, fishery.yrs)
  s.sample.size        <- rep(0, fishery.yrs)

  #Arrays to Store the Estimation Results from SS
  SB          <- array(NA, dim=c(fishery.yrs, ass.num))
  Bratio      <- array(NA, dim=c(fishery.yrs, ass.num))
  TotBio      <- array(NA, dim=c(fishery.yrs, ass.num))
  VulBioEst   <- array(NA, dim=c(fishery.yrs, ass.num))
  Recruits    <- array(NA, dim=c(fishery.yrs, ass.num))
  ...
  F.selex.1.adj<- array(NA, dim=c(1, ass.num))
