...
#Catch
	...
	text(end.yr + 14, max(y.max), label[1], pos = 2, font = 2, col = 1, cex = lab.size) 
...
# No reduction in sampling data scenario
	...
	# Survey
		...
		text(end.yr + 22,  max(y.max) - 2, label[2], pos=2, col=1, font=2, cex=lab.size)
	#Fishery Lengths
		lines(f.lens,  rep(max(y.max) - 3, length(f.lens)), type="l", lwd=fl.size, col=linecol2)
		text(end.yr + 25,  max(y.max) - 3, label[3], pos=2, col=1, font=2, cex=lab.size)
	#Survey Lengths
		points(s.lens, rep(max(y.max) - 4, length(s.lens)), type="p", pch = 16, lwd=far.size, col=linecol3)
		text(end.yr + 24,  max(y.max) - 4, label[4], pos=2, col=1, font=2, cex=lab.size)
	#Fishery Ages
		lines(f.ages,  rep(max(y.max) - 5, length(f.ages)), type="l", lwd=fl.size, col=linecol2)
		text(end.yr + 22,  max(y.max) - 5, label[5], pos=2, col=1, font=2, cex=lab.size)
	#Survey Ages
		points(s.lens, rep(max(y.max) - 6, length(s.ages)), type="p", pch = 16, lwd=far.size, col=linecol3)
		text(end.yr + 21,  max(y.max) - 6, label[6], pos=2, col=1, font=2, cex=lab.size)
