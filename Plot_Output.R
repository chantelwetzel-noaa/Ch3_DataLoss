...
drive = "D:"
run.name = "CPUE_smallN_AE" #"April16_PreCPUE"#"Fall2015"
...
#setwd("C:/PhD/Chapter3/CPUE_smallN_AE/plots")
#setwd("C:/PhD/Chapter3/Plots")
setwd(paste0(drive,"/PhD/Chapter3/", run.name, "/Plots"))


png(filename = "RE_SSB_full.png", width = 6.7, height = 5, units = 'in', res = 256)

par(mfrow= c(2,2), mar = c(0.2,0.2,0.2,0.2), oma = c(4,4,2,4), cex.axis = 1.1, cex.lab = 1.1)
...
par(mfrow= c(1,1), mar = c(0.2,0.2,0.2,0.2), oma = c(4,4,2,4), cex.axis = 1.1, cex.lab = 1.1)
letter.cex = 1; axis.cex = 1.2; label.cex = 0.8
ass.plot.yrs = ifelse(ass.freq == 8, 13, 26)
min = -1; max = 1

to.plot = ass.yrs[1:ass.plot.yrs]
  boxplot(t(med.out[[1]]$re.time.over), ylim = c(min, max), col = rep('grey',length(to.plot)), axes= F)
  box(); abline(h = 0)
  if( a ==1 || a==4) { axis(side = 2)
...
for(a in 1:ds){
  boxplot(t(med.out[[1]]$re.catch[a,,]), ylim = c(min, max), col = rep('grey',length(to.plot)), axes= F)
  box(); abline(h = 0)
  if( a ==1 || a==4) { axis(side = 2)
...
ass.plot.yrs = ifelse(ass.freq == 8, 13, 26)
min = 0; max = max(med.out[[1]]$rmse.ssb.ass[1,])
pch.vec = rep(16:18,2)
...
for (a in 1:ds){
  ...
  lines(index, med.out[[1]]$rmse.ssb.ass[a,], lwd = 1, col = a)
  points(index, med.out[[1]]$rmse.ssb.ass[a,], pch = pch.vec[a], col = a)
}
...
#=========================================================================================================
png(filename = "RMSE_depl_full.png", width = 6.7, height = 3, units = 'in', res = 256)

par(mfrow= c(1,1), mar = c(0.2,0.2,0.2,0.2), oma = c(4,4,2,4), cex.axis = 1.1, cex.lab = 1.1)
letter.cex = 1; axis.cex = 1; label.cex = 1
ass.plot.yrs = ifelse(ass.freq == 8, 13, 26)
min = 0; max = max(med.out[[1]]$rmse.depl) + 5
pch.vec = rep(16:18,2)
...
for (a in 1:ds){
  ...
  if (a == 1 ) { #|| a == 4){
    ...
    if (a == 1) { mtext(side = 1, "Assessment Year", outer = T, cex = label.cex, line = 2)
                  axis(side = 2, cex = axis.cex)
                  mtext(side = 2, "RMSE Depletion", cex = label.cex, outer = T, line = 2)
                  print.letter(alpha.label[a], xy = c(0.10, 0.95), cex = 1.1)  }
    if( a == 4) { print.letter(alpha.label[a-2], xy = c(0.10, 0.95), cex = 1.1)  }
  }
  lines(index, med.out[[1]]$rmse.depl[a,], lwd = 1, col = a)
  points(index, med.out[[1]]$rmse.depl[a,], pch = pch.vec[a], col = a)
}
...
for(b in 1:ds){
  ...
  boxplot(t(est.out[[1]]$h.est[b,,]), ylab = "Steepness", ylim = c(ymin,ymax), col = rep('grey',ass.num), axes = F)
...
#Selectivity Plots ==========================================================================
#=========================================================================================================
png(filename = "FisherySelect.png", width = 6.7, height = 5, units = 'in', res = 256)
par(mfrow = c(ds,2), mar = c(1,1,1,1), oma = c(4,4,4,4))

for(a in 1:ds){
  boxplot(t(med.out[[1]]$re.f.selex[a,1,,]), ylim = c(-0.10, 0.10), axes = F)
  abline(h = 0,col = 1, lty =2); box()
  if (a == 1) { mtext(side = 3, "Fishery Peak") }
  if(a == ds) { axis(side = 1, at =1:length(ass.yrs), label = ass.yrs)}

  boxplot(t(med.out[[1]]$re.f.selex[a,2,,]), ylim = c(-0.10, 0.10), axes = F)
  abline(h = 0, col = 1, lty =2); box()
  if (a == 1) { mtext(side = 3, "Fishery Top") }
  if(a == ds) { axis(side = 1, at =1:length(ass.yrs), label = ass.yrs)}

  boxplot(t(med.out[[1]]$re.f.selex[a,3,,]), ylim = c(-0.10, 0.10), axes = F)
  abline(h = 0, col = 1, lty =2); box()
  if (a == 1) { mtext(side = 3, "Fishery Top") }
  if(a == ds) { axis(side = 1, at =1:length(ass.yrs), label = ass.yrs)}

  boxplot(t(med.out[[1]]$re.f.selex[a,4,,]), ylim = c(-0.10, 0.10), axes = F)
  abline(h = 0, col = 1, lty =2); box()
  if (a == 1) { mtext(side = 3, "Fishery Ascending") }
  if(a == ds) { axis(side = 1, at =1:length(ass.yrs), label = ass.yrs)}

  if(a == 1){ mtext(side =2, "Relative error",  outer = T, line = 2)
              mtext(side =1, "Assessment Year", outer = T, line = 2)}
}
...
ymax = ceiling( max(med.out[[1]]$med.ssb[,start.plot:final.yr,]))
ymax = 0.75
ass.index = rev(c(1, 2, 3, 4, 5)) #(rev(c(1, 8, 13, 20, 26))
...
for(b in 1:ds){
  ...
  #print.letter(alpha.label1[b], xy = c(0.04, 0.95))
  boxplot(t(med.out[[1]]$re.m[b,,]), ylab = "RE M", ylim = c(ymin,ymax), col = rep('grey',ass.num), axes = F)
...
for(b in 1:ds){
  xfact = 0.10
  boxplot(t(est.out[[1]]$s.selex.est[b,1,,]), ylim = c(39 - xfact*39, 39 + xfact*39))
  abline(h = 39,col = 1, lty =2)
  mtext(side = 3, "Survey Peak")
  boxplot(t(est.out[[1]]$s.selex.est[b,3,,]), ylim = c(2, 5))
  abline(h = 4.25, col = 1, lty =2)
  mtext(side = 3, "Survey Asc")
  
  xfact = 0.05
  boxplot(t(est.out[[1]]$f.selex.est[b,1,,]), ylim = c(45 - xfact*45, 45 + xfact*45))
  abline(h = 45,col = 1, lty =2)
  mtext(side = 3, "Fishery Peak")
  boxplot(t(est.out[[1]]$f.selex.est[b,3,,]), ylim = c(2, 5))
  abline(h = 4.25,col = 1, lty =2)
  mtext(side = 3, "Fishery Asc")
  boxplot(t(est.out[[1]]$f.selex.adj.est[b,1,,]), ylim = c(-10, 10))
