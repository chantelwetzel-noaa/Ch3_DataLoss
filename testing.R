...
wd = paste("C:/PhD/Chapter3/",LH,"_ds_survey_50yr_sims_1_100/save/",sep="") 
...
boxplot(re.sb1, ylim = c(-0.5, 0.5)); abline(h = 0)
boxplot(re.time.over, ylim =  c(-1, 1)); abline(h = 0)



grey = rgb(0,0,0,0.10)
par(mfrow =c (2, length(ass.index)), oma = c(4,4,2,4), mar = c(2,2,2,2))
for (a in 1:length(ass.index)){
  ...
  plot( temp, med.re.ssb[2,], type = 'l', lwd =2, ylim = c( -1, 1), axes = F)# xlim = c(1, max(50 + ass.index.flat* 4 - 4)) )
...
for (a in 1:length(ass.index)){
  ...
  plot( temp, med.re.depl[2,], type = 'l', lwd =2, ylim = c( -1, 1), axes = F) 
...
plot(1:ss.years, med.rec[1:ss.years], type ='l', lwd =2, ylim = c(0, 10000), ylab = "Recruits")
for (a in 1:ass.length){
	ind = 1:(first.ass + a*4 -4)
	if (a == ass.length) { ind = 1:(first.ass+a*4-5) }
