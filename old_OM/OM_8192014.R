...
#source("F://PhD//Chapter3//Code//OM_7072014.R") 
drive <-"F:"
LH <- "rockfish"
SR <- "BH" #"Shep" 
sigma <- 0.36
...
#Source in the data scenarios
 ...
 rho      <- 0 
 if (data.scenario == "normal_auto") { rho = 1 / sqrt(2) }
...
#Project with the estimated harvest----------------------------------------------------------------------------------------
 ...
 for (y in (pre.fishery.yrs + setup.yrs):total.yrs) {
    ...
    if (data.scenario == "normal_estM" || data.scenario == "normal_auto") {
        if (decl.overfished  == TRUE) {
            ind             <- y - pre.fishery.yrs
