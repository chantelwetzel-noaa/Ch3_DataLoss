...
#/usr/bin/R-64-v3.0.0
#source("//even_more_home//h_cwetzel//PhD//Chapter3//code//OM_8272014.R") 
#source("F://PhD//Chapter3//Code//OM_8272014.R") 
drive <-"F:" #"//home//cwetzel//h_cwetzel"
LH <- "flatfish"
SR <- "BH" #"Shep" 
sigma <- 0.36
p.value <- 0.45
...
#Source in the data scenarios
 ...
 select.err   <- rnorm(fishery.yrs, 0, 0.05)
 inflec.selec <- numeric(fishery.yrs)
 ...
 Update_Dynamics <- function(R0, catch = hist.catch, biology)
 {
    ...
    inflec.selec[y] <- fsp1 * exp(-0.50 * 0.05 * 0.05) * exp(select.err[y])
    selec.out       <- TimeVarying_Selex(mid.phi.m = biology$mid.phi.m, mid.phi.f = biology$mid.phi.f, selec = inflec.selec[y])
    ...
    names(UpdateDyn) <- c("f.values","catch.wght.values","catch.at.age","catch.at.len","numbers","SSB","Ry", "selec", "selec.age")
    return(UpdateDyn)
 }
...
#Format Data ----------------------------------------------------------------------------------------------------------
 ...
 survey.out     <- Do_Survey(biology = Get_Biology(), f.values = Dyn$f.values , numbers = Dyn$numbers, 
                             index = survey.dur,  selec.age.f = Get_Biology()$selec.age.f, selec.age.m =Get_Biology()$selec.age.m)
...
#Project with the estimated harvest----------------------------------------------------------------------------------------
 ...
 for (y in (pre.fishery.yrs + setup.yrs):total.yrs) {
    ...
    #Time-Varying Selectivity---------------------------------------------------------------------------------------------------------
    fsp1.realized[y]<- peak
    inflec.selec[y] <- fsp1.realized * exp(-0.50 * 0.05 * 0.05) * exp(select.err[y])
    selec.out       <- TimeVarying_Selex(mid.phi.m = biology$mid.phi.m, mid.phi.f = biology$mid.phi.f, selec = inflec.selec[y])
    selec.age.m     <- selec.out$selec.age.m
    selec.age.f     <- selec.out$selec.age.f
    selec[y,]       <- selec.out$selec
    ...
    if ( do.ass %% 4 == 0 ){
        counter = counter + 1
        if (decl.overfished == TRUE && need.blocks == TRUE) { end.yr = y - pre.fishery.yrs -1
          block.yrs = c(decl.yr, end.yr)
          block.pattern = 1
          if (overfished.counter > 1) { 
            block.pattern = block.pattern + 1
            block.yrs = c(init.decl.yr, init.end.yr, decl.yr, end.yr) } }
        ...
        if (decl.overfished == FALSE) {
            if(Bratio[ind,counter] < over.thres) {
              ...
              if (overfished.counter > 1) { init.decl.yr = decl.yr  ; init.end.yr = end.yr }
              need.blocks = TRUE
              block.num = 1 ; block.fxn = 2
              if (data.scenario == "greathist") { need.blocks = FALSE
              block.num = 0; block.fxn = 0 }
              decl.yr = y - pre.fishery.yrs + 1 #The end year block is set above
              #selec.out   <- Overfished_Selex(mid.phi.m = biology$mid.phi.m, mid.phi.f = biology$mid.phi.f)
              #selec.age.f <- selec.out$over.selec.age.f
              #selec.age.m <- selec.out$over.selec.age.m
              #selec       <- selec.out$over.selec
              #peak        <- peak + selec.adj
            }
        }
        ...
        if (decl.overfished == TRUE) {
            if(Bratio[ind,counter] >= ctl.rule.tgt) {
              decl.overfished <- FALSE
              end.yr = y - pre.fishery.yrs -1 #This is for the blocks
    ...
    Proj[[16]]<- f.age.samp
    Proj[[17]]<- s.age.samp
    Proj[[18]]<- fsp1.realized

    names(Proj) <- c("f.values", "catch.wght.values", "catch.at.age", "catch.at.len", "numbers", "SSB", "depl","Ry", "index.expect",
                "vul.total.obs", "fore.catch","true.ofl", "true.f","f.len.samp","s.len.sam","f.age.samp","s.age.samp", "fsp1.realized")
