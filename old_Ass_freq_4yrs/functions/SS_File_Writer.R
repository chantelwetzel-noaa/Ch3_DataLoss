...
{   
    ...
    for (i in 2:y){
        block.selec[i-1,] <- c(25, 60,  fsp1.vec[i], fsp1.vec[i] ,  -1,     1,  3,  paste("#Selec1_", i))
    }
    fsp2.block <- matrix(0, y - 1, 8)
    for(i in 2:y){
       fsp2.block[i-1,] <- c(-10, 10,  fsp2.vec[i], fsp2.vec[i] ,  -1,     1,  3,  paste("#Selec2_", i)) 
    }
    ...
    k.matrix <- matrix(0, y-1, 8)
    for (i in 2:y){
        k.matrix[i-1,] <- c(0.01, 0.20,  k.vec[i], k.vec[i],  1,  0.20,  3,  paste("#k", i))
    }
    ...
    #write.table(k.matrix, file = ctl, append = T, row.names = FALSE, col.names = FALSE, quote = FALSE)
    ...
    if (do.cpue){
        if (cpue.q > 0){ cat(-6, 6, log(0.01), log(0.01), -1, 99, -1, " #Fishery CPUE\n",
                file = ctl, append = T) }       
    }

    if (fix.q > 0){ cat(-6, 6, 0, 0, -1, 99, -1, " #Survey\n",
                file = ctl, append = T) }

    if(OM.run.1) { cat(-6, 6, 0, 0, -1, 99, -1,  " #Depletion\n",
              file = ctl, append = T) }
    ...
    write.table(fsp2.block,  file = ctl, append=T, row.names = F, col.names = F, quote=F) 
...
{   
    ...
    #_LO    HI   INIT   PRIOR  PR_type SD  PHASE
    -10,    10,  fsp2,  fsp2,  -1,     1,  3,  "#Peak Block"),
    ncol = 8, byrow = F) 

    block.selec.2 <- matrix(c(
    #_LO    HI   INIT   PRIOR  PR_type SD  PHASE
    -10,     10,  fsp2,  fsp2,  -1,     0.50,  3,  "#Dome Block"),
    ncol = 8, byrow = F)  
    ...
    if (cpue.q > 0){ cat(-6, 6, -2.3, -2.3, -1, 99, -1,
                file = ctl, append = T) }
    ...
    if(need.blocks == T){
        ...
        for (z in 1:block.pattern){
           write.table(block.selec.2, file = ctl, append=T, row.names = F, col.names = F, quote=F) }
    }
...
writeDat<-function(dat, y, survey, fore.catch)
{
    ...
    survey.obs = ifelse(do.cpue == F, dim(ss.survey.data)[1] + dim(depl.survey)[1], dim(ss.survey.data)[1] + dim(depl.survey)[1] + dim(cpue.data)[1] )
    cat("#\n",
    survey.obs, " #Number of Survey Observations\n",
