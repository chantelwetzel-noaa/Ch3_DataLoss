...
drive = "C:"
run.name = "Ass_Freq"#"CPUE_smallN_AE" #"April16_PreCPUE"#"Fall2015"
...
for(a in 1:ds){
  for (b in 1:N){
    plot.matrix[a,b] = ifelse(est.out[[1]]$time.over[a,b] == -101, 101, est.out[[1]]$time.over[a,b])
  }
}
...
box()
axis(side = 2)
mtext(side = 2, outer = T, "Total Overfished Catch", line = 2.5)
#axis(side = 1, at = 1:length(to.plot), labels = to.plot - pre.yrs + 1)
mtext(side = 1, outer = T, "Scenario", line = 2.5)
...
for(a in 1:ds){
  ...
  if( a > 3){
    axis(side = 1, at = seq(50, max(limit.yrs), 10))
    mtext(side = 1, outer = T, "Assessment year", line = 2.5)
  }
  if (a < 4)  { mtext(side = 3, outer = F, name.label[a]) }
  if (a == 3 ){ mtext(side = 4, outer = F, "Time-invariant", line = 1) }
...
ass.plot.yrs = ifelse(ass.freq == 8, 13, 26)
min = 0; max = max(med.out[[1]]$rmse.ssb.ass[1,]+5)
pch.vec = rep(16:18,2)
...
for (a in 1:ds){
  ...
  if (a == 1 || a == 4){
    ...
    if (a == 1) { mtext(side = 1, "Assessment Year", outer = T, cex = label.cex, line = 2)
                  ...
                  print.letter(alpha.label[a], xy = c(0.05, 0.95), cex = 1.1)  }
    if( a == 4) { print.letter(alpha.label[a-2], xy = c(0.05, 0.95), cex = 1.1) 
...
for(b in 1:ds){
  ...
  if (b == ds) { mtext(side = 1, "Assessment Year", outer = T, line = 2.5) 
       mtext(side = 2, "RE Steepness", outer =T, line = 2.5) }
}
dev.off()
...
par(mfrow = c(ds,4), mar = c(1,1,1,1), oma = c(4,4,4,4))
ymin = -1; ymax = 1
...
for(a in 1:ds){
  ...
  boxplot(t(med.out[[1]]$re.f.selex[a,2,,]), ylim = c(ymin, ymax), axes = F)
  abline(h = 0, col = 1, lty =2); box()
  if (a == 1) { mtext(side = 3, "Fishery Top") }
  if(a == ds) { axis(side = 1, at =1:length(ass.yrs), label = ass.yrs)}

  boxplot(t(med.out[[1]]$re.f.selex[a,3,,]), ylim = c(ymin, ymax), axes = F)
  abline(h = 0, col = 1, lty =2); box()
  if (a == 1) { mtext(side = 3, "Fishery Top") }
  if(a == ds) { axis(side = 1, at =1:length(ass.yrs), label = ass.yrs)}

  boxplot(t(med.out[[1]]$re.f.selex[a,4,,]), ylim = c(ymin, ymax), axes = F)
  abline(h = 0, col = 1, lty =2); box()
